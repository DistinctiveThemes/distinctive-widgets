...
class dt_custom_query extends WP_Widget {
    ...
    function dt_custom_query() {
        $widget_ops = array( 'classname' => 'dt_custom_query', 'description' => 'Query custom entries' );
        $control_ops = array( 'width' => 520, 'height' => 350, 'id_base' => 'dt_custom_query' );
        parent::__construct( 'dt_custom_query', 'DistinctiveThemes: Custom Content Query', $widget_ops, $control_ops);
    }
                ...
                <div class="<?php if ( $postcls ) { echo $postcls; } else { echo 'featuredpost'; } if($count == $postnr) { echo ' lastpost'; } ?>">
					...
					if ( $d_thumb ) {
						if ( $swap ) { ?>
							...
							if (function_exists('dt_media')) {
								dt_media(array(
									'name' => 'distinctive_themes-widget-pack',
									'imgtag' => 1,
									'link' => 1,
									'enable_video' => $video,
    ...
    function form( $instance ) {
        ...
        <p>
			<label for="<?php echo $this->get_field_id('c_image'); ?>"><?php _e('Section or Category Image URL (replaces text title)','distinctive_themes-widget-pack'); ?> <span style="color:#aaa"><?php _e('ex: //www.domain.com/image.jpg)', 'distinctive_themes-widget-pack'); ?></span></label> 
        ...
        <p>
			<label for="<?php echo $this->get_field_id('c_link'); ?>"><?php _e('Link for Section or Category title','distinctive_themes-widget-pack'); ?> <span style="color:#aaa"><?php _e('(leave empty for to disable)','distinctive_themes-widget-pack'); ?></span></label> 
        ...
        <p>
            <label for="<?php echo $this->get_field_id('cat_or_postpage'); ?>"><?php _e('Do you want to run a query based on Category or Post/Page ID','distinctive_themes-widget-pack'); ?></label><br />
            <select id="<?php echo $this->get_field_id( 'cat_or_postpage' ); ?>" name="<?php echo $this->get_field_name( 'cat_or_postpage' ); ?>" style="width:480px">
                <option value="1" <?php if ( '1' == $instance['cat_or_postpage'] ) echo 'selected="selected"'; ?>><?php _e('Category','distinctive_themes-widget-pack'); ?></option>
                <option value="0" <?php if ( '0' == $instance['cat_or_postpage'] ) echo 'selected="selected"'; ?>><?php _e('Post/Page','distinctive_themes-widget-pack'); ?></option>    
            </select>
        </p>
		
        <p>
            <label for="<?php echo $this->get_field_id('postids'); ?>"><?php _e('Enter <a href="//www.distinctive_themesthemes.com/how-to-check-category-ids/">Category ID(s)</a> - OR - Post/Page ID(s)','distinctive_themes-widget-pack'); ?> <span style="color:#aaa"><?php _e('(ex:3,5,99 comma separated, no spaces)','distinctive_themes-widget-pack'); ?></span></label>
        ...
        <p>
            <label for="<?php echo $this->get_field_id('post_order'); ?>"><?php _e('Post Order: only if category option is selected as base for query','distinctive_themes-widget-pack'); ?></label><br />
        ...
        <p style="background-color: #efefef;border:1px solid #ddd;padding:10px;"><?php _e('Custom query will generate thumbnails only if TimThumb is the active thumbnail script. All DistinctiveThemes themes have TimThumb option enabled by default. However, if you use WP Post Thumbnails option, then thumbnails CANNOT be generated by this widget.','distinctive_themes-widget-pack'); ?></p>
        ...
        <p style="float:right;width:235px">
            <label for="<?php echo $this->get_field_id( 'video' ); ?>"><?php _e('Show Videos','distinctive_themes-widget-pack'); ?></label><br />
            <select id="<?php echo $this->get_field_id( 'video' ); ?>" name="<?php echo $this->get_field_name( 'video' ); ?>" style="width:235px">
                <option value="1" <?php if ( '1' == $instance['video'] ) echo 'selected="selected"'; ?>><?php _e('Yes','distinctive_themes-widget-pack'); ?></option>
                <option value="0" <?php if ( '0' == $instance['video'] ) echo 'selected="selected"'; ?>><?php _e('No','distinctive_themes-widget-pack'); ?></option>    
            </select>
        </p>   		
		
        <p style="float:left;width:235px">
            <label for="<?php echo $this->get_field_id('media_w'); ?>"><?php _e('Width of Thumbnail (ex: 50 or 300)','distinctive_themes-widget-pack'); ?></label>
        ...
        <p style="float:right;width:235px">
            <label for="<?php echo $this->get_field_id('media_h'); ?>"><?php _e('Height of Thumbnail (ex: 50 or 300)','distinctive_themes-widget-pack'); ?></label>
        ...
        <p style="float:right;width:235px">
            <label for="<?php echo $this->get_field_id('excerpt_l'); ?>"><?php _e('# of Words in Excerpt (ex:35 / Max: 55)','distinctive_themes-widget-pack'); ?></label>
        ...
        <p>
			<label for="<?php echo $this->get_field_id( 'swap' ); ?>"><?php _e('Thumbnail and Title Position','distinctive_themes-widget-pack'); ?></label><br />
			<select id="<?php echo $this->get_field_id( 'swap' ); ?>" name="<?php echo $this->get_field_name( 'swap' ); ?>" style="width:480px">
				<option value="1" <?php if ( '1' == $instance['swap'] ) echo 'selected="selected"'; ?>><?php _e('Show title first then thumbnail','distinctive_themes-widget-pack'); ?></option>
				<option value="0" <?php if ( '0' == $instance['swap'] ) echo 'selected="selected"'; ?>><?php _e('Show thumbnail first then title','distinctive_themes-widget-pack'); ?></option>
			</select>
		</p>			
		        
        <p>
            <label for="<?php echo $this->get_field_id( 'postmeta' ); ?>"><?php _e('Display post meta below excerpt','distinctive_themes-widget-pack'); ?></label> 
		...
		<h4><?php _e('Advanced - For Developers Only','distinctive_themes-widget-pack'); ?></h4>
