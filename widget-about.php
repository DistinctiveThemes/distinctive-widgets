...
class distinctive_themes_about extends WP_Widget {

	function distinctive_themes_about() {
		$widget_ops = array( 'classname' => 'dt_about_widget', 'description' => 'Display an "about" widget' );
		$control_ops = array( 'width' => 400, 'height' => 350, 'id_base' => 'dt_about_widget' );
		parent::__construct( 'dt_about_widget', 'DistinctiveThemes: About', $widget_ops, $control_ops);	
	}
	...
	<p>
		<label for="<?php echo $this->get_field_id( 'a_avatar' ); ?>"><?php _e('Site Admin\'s Avatar','distinctive_themes-widget-pack'); ?></label> 
	...
	<p>
		<label for="<?php echo $this->get_field_id('a_text'); ?>"><?php _e('About Text','distinctive_themes-widget-pack'); ?></label>
	...
	<p>
		<label for="<?php echo $this->get_field_id('a_imgurl'); ?>"><?php _e('URL to Display a Custom Image','distinctive_themes-widget-pack'); ?></label>
	...
	<p>
		<label for="<?php echo $this->get_field_id( 'a_align' ); ?>"><?php _e('If a Custom Image Defined - Align Image','distinctive_themes-widget-pack'); ?></label><br />
		<select id="<?php echo $this->get_field_id( 'a_align' ); ?>" name="<?php echo $this->get_field_name( 'a_align' ); ?>" style="width:235px">
			<option value="alignleft" <?php if ( 'alignleft' == $instance['a_align'] ) echo 'selected="selected"'; ?>><?php _e('Left','distinctive_themes-widget-pack'); ?></option>
			<option value="alignright" <?php if ( 'alignright' == $instance['a_align'] ) echo 'selected="selected"'; ?>><?php _e('Right','distinctive_themes-widget-pack'); ?></option>
			<option value="aligncenter" <?php  if ( 'aligncenter' == $instance['a_align'] ) echo 'selected="selected"'; ?>><?php _e('Center','distinctive_themes-widget-pack'); ?></option>            
		</select>
	</p>	

	<p>
		<label for="<?php echo $this->get_field_id('a_link'); ?>"><?php _e('Post or Page ID for link','distinctive_themes-widget-pack'); ?></label>
...
function register_distinctive_themes_about() {
	register_widget('distinctive_themes_about');
}

add_action('widgets_init', 'register_distinctive_themes_about');