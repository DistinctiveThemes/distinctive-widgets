...
class distinctive_themes_feedburner extends WP_Widget {

	function distinctive_themes_feedburner() {
		$widget_ops = array( 'classname' => 'distinctive_themes_feedburner_widget', 'description' => 'Feedburner Email Subscribe');
		$control_ops = array( 'width' => 250, 'height' => 350, 'id_base' => 'distinctive_themes_feedburner_widget' );
		parent::__construct( 'distinctive_themes_feedburner_widget', 'DistinctiveThemes: Feedburner Email Subscribe', $widget_ops, $control_ops);
	}	
		...
		$bgurl	= $instance['bgurl'];
			...
			echo $before_widget;
				...
				if ($wstyle == "small") {
					...
					<form class="distinctive_themes_f_widget" action="//feedburner.google.com/fb/a/mailverify" method="post" target="popupwindow" onsubmit="window.open('//feedburner.google.com/fb/a/mailverify?uri=<?php echo esc_attr( $user ); ?>', 'popupwindow', 'scrollbars=yes,width=550,height=520');return true">
						<fieldset <?php if ( $bgcol ) { echo 'style="background:' . esc_attr( $bgcol ) .';border:1px solid ' . esc_attr( $bordercol ) .';"'; } ?>>
							...
							<input type="image" class="feedburner_submit" src="<?php echo get_template_directory_uri(); ?>/framework/images/add.png" alt="<?php _e('Subscribe', 'distinctive_themes-widget-pack'); ?>" />
						</fieldset>
					</form>
				<?php } else { ?>
					...
					<div class="news-signup" <?php if ( $bgurl ) { echo 'style="background:url(' . esc_attr( $bgurl ) .') repeat top center"'; } ?>>	
						...
						<form class="sidebar_feedwidget" action="//feedburner.google.com/fb/a/mailverify" method="post" target="popupwindow" onsubmit="window.open('//feedburner.google.com/fb/a/mailverify?uri=<?php echo esc_attr( $user ); ?>', 'popupwindow', 'scrollbars=yes,width=550,height=520');return true">
							...
							<input type="hidden" value="<?php echo esc_attr( $user ); ?>" name="uri" />
							<input type="hidden" name="loc" value="<?php bloginfo('language'); ?>"/>
								<input type="image" class="sidebar_mailinput_submit" src="<?php echo plugins_url(); ?>/distinctive_themes-widget-pack/images/submit-newsletter-add.png" alt="<?php _e('Subscribe by mail','distinctive_themes'); ?>" alt="<?php _e('Subscribe', 'distinctive_themes-widget-pack'); ?>" />
							</fieldset>
						</form>
	...
	function form( $instance ) {
		...
		<p>
			<label for="<?php echo $this->get_field_name( 'wstyle' ); ?>"><?php _e('Select subscribe form size','distinctive_themes-widget-pack'); ?></label>
			<select id="<?php echo $this->get_field_id( 'wstyle' ); ?>" name="<?php echo $this->get_field_name( 'wstyle' ); ?>">
				<option value="small" <?php if ( 'small' == $instance['wstyle'] ) echo 'selected="selected"'; ?>><?php _e('Small Form','distinctive_themes-widget-pack'); ?></option>
				<option value="big" <?php if ( 'big' == $instance['wstyle'] ) echo 'selected="selected"'; ?>><?php _e('Big Form','distinctive_themes-widget-pack'); ?></option>    
			</select>
		</p>		
		...
		<p>
			<label for="<?php echo $this->get_field_id('user'); ?>"><?php _e('Feedburner ID','distinctive_themes-widget-pack'); ?></label>
		...
		<p>
			<label for="<?php echo $this->get_field_id('text'); ?>"><?php _e('Text','distinctive_themes-widget-pack'); ?></label>
		...
		<p style="background-color: #efefef;border:1px solid #ddd;padding:10px;"><?php _e('Adjust style of input field for small subscribe form.','distinctive_themes-widget-pack'); ?></p>
		
		<p>
			<label for="<?php echo $this->get_field_id('bgcol'); ?>"><?php _e('Input field background color','distinctive_themes-widget-pack'); ?></label>
		...
		<p>
			<label for="<?php echo $this->get_field_id('bordercol'); ?>"><?php _e('Input field border color','distinctive_themes-widget-pack'); ?></label>
		...
		<p>
			<label for="<?php echo $this->get_field_id('textcol'); ?>"><?php _e('Input field text color','distinctive_themes-widget-pack'); ?></label>
		...
		<p>
			<label for="<?php echo $this->get_field_id('bgurl'); ?>"><?php _e('URL for custom background','distinctive_themes-widget-pack'); ?></label>
...
function register_distinctive_themes_feedburner() {
	register_widget('distinctive_themes_feedburner');
}

add_action('widgets_init', 'register_distinctive_themes_feedburner');