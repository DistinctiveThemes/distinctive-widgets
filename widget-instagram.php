...
class distinctive_themes_flickrrss extends WP_Widget {
	...
	public function widget( $args, $instance ) {
		...
		echo $before_widget;
			...
			if ( $title ) {
				...
				function distinctive_themes_instagramphoto($value)
				{
					...
					return "<div class='distinctive_themes_instagram_thumb'><a href='$link' target='_blank' rel='nofollow'><img itemprop='image' src='$thumb'/></a></div>";
				}
				...
				{
					echo distinctive_themes_instagramphoto( $value );
				}
	...
	function form( $instance ) {
		$defaults	= array(
			'title'         => __('Instagram Feed', 'distinctive_themes-widget-pack'),
			'photos_of'     => __('username', 'distinctive_themes-widget-pack'),
			'instagram_uid' => '',
			'instagram_cid' => '',
			'instagram_tag' => '',
			'access_token'  => '',
		...
		<div class="controlpanel">
			...
			<p>
				<label for="<?php echo $this->get_field_id( 'photos_of' ); ?>"><?php _e('Display photos by','distinctive_themes-widget-pack'); ?></label> 
				<select class="widefat" id="<?php echo $this->get_field_id( 'photos_of' ); ?>" name="<?php echo $this->get_field_name( 'photos_of' ); ?>">
					<option value="username" <?php if ( 'username' == $instance['photos_of'] ) echo 'selected="selected"'; ?>><?php _e('Username','distinctive_themes-widget-pack'); ?></option>
					<option value="hashtag" <?php  if ( 'hashtag' == $instance['photos_of'] ) echo 'selected="selected"'; ?>><?php _e('Hashtag','distinctive_themes-widget-pack'); ?></option>
				</select>
			</p>

			<p>
			<?php
			printf(__('To display user photos, an access token, User and Client ID\'s are required. To get them, first you\'ll need to <a href="%1$s" target="_blank">register an application</a> then copy Client ID. Then visit <a href="%2$s" target="_blank">this</a> site to get User ID and access token', 'distinctive_themes-widget-pack'), 'https://instagram.com/developer/', 'https://smashballoon.com/instagram-feed/token/' );	?>				
			</p>

			<p>
				<label for="<?php echo $this->get_field_id( 'instagram_uid' ); ?>"><?php _e('User ID','distinctive_themes-widget-pack'); ?></label>
			...
			<p>
				<label for="<?php echo $this->get_field_id( 'instagram_cid' ); ?>"><?php _e('Client ID','distinctive_themes-widget-pack'); ?></label>
			...
			<p>
				<label for="<?php echo $this->get_field_id( 'instagram_tag' ); ?>"><?php _e('Hashtag','distinctive_themes-widget-pack'); ?></label>
			...
			<p>
				<label for="<?php echo $this->get_field_id( 'access_token' ); ?>"><?php _e('Access Token','distinctive_themes-widget-pack'); ?></label>
			...
			<p>
				<label for="<?php echo $this->get_field_name( 'photo_count' ); ?>"><?php _e('Number of photos','distinctive_themes-widget-pack'); ?></label>
