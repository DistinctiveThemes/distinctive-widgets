...
class distinctive_themes_authorbadge extends WP_Widget {

	function distinctive_themes_authorbadge() {
		$widget_ops = array( 'classname' => 'distinctive_themes_authorbadge', 'description' => 'Display widget with a big icon' );
		$control_ops = array( 'width' => 350, 'id_base' => 'distinctive_themes_authorbadge' );
		parent::__construct( 'distinctive_themes_authorbadge', 'DistinctiveThemes: Author Badge', $widget_ops, $control_ops);	
	}
	...
	public function widget($args, $instance) {
		...
		echo $before_widget;
				?>
					<p class="first_p"><?php printf( esc_attr__( 'About %s', 'distinctive_themes-widget-pack' ), get_the_author() ); ?></p>
	...
	?>
		...
		<p class="distinctive_themes_widgetinfo">
			<?php _e('Author badge shows at single post and author pages only. This badge will not shown if Author bio is left empty.','distinctive_themes-widget-pack'); ?>
		<p>
				
		<strong><?php _e('Link Label:','distinctive_themes-widget-pack'); ?></strong>
		<div style="background-color: #efefef;border:1px solid #ddd;padding:9px;overflow:hidden;margin-bottom:10px">
			<div style="float:left;width:163px;">
				<p>
					<label for="<?php echo $this->get_field_id('social_heading'); ?>"><?php _e('Social Links Heading','distinctive_themes-widget-pack'); ?></label>
				...
				<p>
					<label for="<?php echo $this->get_field_id('f_facebook'); ?>"><?php _e('Facebook Link','distinctive_themes-widget-pack'); ?></label>
				...
				<p>
					<label for="<?php echo $this->get_field_id('f_google'); ?>"><?php _e('Google+ Link','distinctive_themes-widget-pack'); ?></label>
			...
			<div style="float:right;width:163px;">
				<p>
					<label for="<?php echo $this->get_field_id('f_viewwebsite'); ?>"><?php _e('Author\'s Website','distinctive_themes-widget-pack'); ?></label>
				...
				<p>
					<label for="<?php echo $this->get_field_id('f_allposts'); ?>"><?php _e('Author\'s Posts','distinctive_themes-widget-pack'); ?></label>
				...
				<p>
					<label for="<?php echo $this->get_field_id('f_twitter'); ?>"><?php _e('Twitter Link','distinctive_themes-widget-pack'); ?></label>
        ...
        <p>
            <label for="<?php echo $this->get_field_id('picsize'); ?>"><?php _e('The avatar size','distinctive_themes-widget-pack'); ?></label>
		...
		<p><?php _e('Go to User profile page to enter Facebook, Twitter and Author URL details','distinctive_themes-widget-pack'); ?></p>
	
<?php
	}
}

function register_distinctive_themes_authorbadge() {
	register_widget('distinctive_themes_authorbadge');
}

add_action('widgets_init', 'register_distinctive_themes_authorbadge');